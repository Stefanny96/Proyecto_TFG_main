...
function agregarLinea() {
    ...
    if(idProveedorSeleccionado != 0) {
        ...
        fila.innerHTML = `
                ...
                <td>
                    <select class="form-control select2 selectProducto" style="width: 100%;">
                ...
                <td>
                    <button type="button" class="btnEliminarFila" onclick="eliminarFila(this)">Eliminar</button>
                </td>
            `;

        document.getElementById("detallePedido").appendChild(fila);

        rellenarSelectProducto(fila);
...
function rellenarSelectProducto(fila){
    let selectFilaProducto = fila.getElementsByTagName("select")[0];
    //console.log(document.getElementById("selectProveedor").value);
    let idProveedorSeleccionado = document.getElementById("selectProveedor").value;

    leerJsonProductos(function (data) {
        // Filtrar productos por el proveedor seleccionado
        let productosFiltrados = data.filter(
          (producto) => producto.proveedor.proveedor_id == idProveedorSeleccionado
        );
        ...
        // Actualizar el select de productos con Select2
        $(selectFilaProducto).empty().select2({
...
function limpiarProducto() {
  let listaTxtDetallesProductos = [
    ...document.getElementsByClassName("txtProductos"),
  ];
  listaTxtDetallesProductos.forEach((txtProduct) => {
    txtProduct.value = ""; //Lo pongo a vacio para que se "limpie"
  });
}
...
function eliminarFila(boton) {
  ...
  if (filas.length == 1) {
    ...
    // Si hay más de una fila, elimina la fila correspondiente
